...
def causal_joint_ot_grid(cost_matrix, chain, kk, ll, mm, metric, processed_pairs, processed_dists, constraints):
    ...
    for i in range(k-1):
        ...
        s            = chain[i].get_child(chain).data.get_domain('base')
        t            = chain[i].get_child(chain).data.get_domain('abst')
        pos_t, pos_s = ut.find_positions(lst_abst,t), ut.find_positions(lst_base,s)
        sub_indices  = np.ix_(pos_t, pos_s)

        omega_mat_base, omega_mat_abst = np.zeros((m, n)), np.zeros((m, n))
        omega_mat_base[:, sub_indices[1]] = 1
        omega_mat_abst[sub_indices[0], :] = 1
        #omega_mat[sub_indices[0], sub_indices[1]] = 1
        
        Z_base = [np.sum([chain[i].data.base_distribution[k] for k in pos_s])]*n
        Z_abst = [np.sum([chain[i].data.abst_distribution[j] for j in pos_t])]*m
        
        curr_plan      = cp.reshape(p, (m, n))
        #norm_curr_plan = cp.multiply(curr_plan, omega_mat)
        norm_curr_plan_base = cp.multiply(curr_plan, omega_mat_base)
        norm_curr_plan_abst = cp.multiply(curr_plan, omega_mat_abst)
        
        next_plan      = cp.reshape(p_next, (m, n))
        
        ot_term, reg_b, reg_a, entropy = cp.Constant(0), cp.Constant(0), cp.Constant(0), cp.Constant(0) 
        ...
        if (chain[i], chain[i+1]) not in processed_dists:
            ...
            reg_b = regularizer(cp.sum(norm_curr_plan_base, axis = 0)/Z_base, cp.sum(next_plan, axis = 0), metric)
            reg_a = regularizer(cp.sum(norm_curr_plan_abst, axis = 1)/Z_abst, cp.sum(next_plan, axis = 1), metric)

            processed_dists.append((chain[i],chain[i+1]))

        obj_i = kappa*ot_term + lmbda*(reg_b + reg_a) - mu*entropy
...
def causal_joint_ot_grid_multi(cost_matrix, chain, kk, ll1, ll2, mm, metric, processed_pairs, processed_dists, constraints):
    ...
    C     = cp.Constant(cost_matrix)
    kappa = cp.Constant(kk)
    lmbda1 = cp.Constant(ll1)
    lmbda2 = cp.Constant(ll2)
    mu    = cp.Constant(mm)
    ...
    for i in range(k-1):
        ...
        obj_i = kappa*ot_term + lmbda1*reg_b + lmbda2*reg_a - mu*entropy
        obj_list.append(obj_i)
...
def causal_joint_ot_grid_parents_multi(cost_matrix, chain, kk, ll1, ll2, mm, metric, processed_pairs, processed_dists, constraints, exp):
...
def causal_joint_ot_grid_approx(cost_matrix, chain, kk, ll, mm, metric, processed_pairs, processed_dists, constraints):
...
def causal_joint_ot_grid_parents_approx(cost_matrix, chain, kk, ll, mm, metric, processed_pairs, processed_dists, constraints, exp):
