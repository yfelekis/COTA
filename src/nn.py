...
class JointNeuralNet(nn.Module):
    def __init__(self, M0, M1, R, a, J, T=0.1, frozen_alphas={}):
        ...
        for i in range(len(self.M0_diagrams)):
            ...
            low_path = (left_alpha, torch.tensor(self.A.M1.compute_mechanisms(self.M1_diagrams[i][0],self.M1_diagrams[i][1]),dtype=torch.float32))
            high_path = (torch.tensor(self.A.M0.compute_mechanisms(self.M0_diagrams[i][0],self.M0_diagrams[i][1]),dtype=torch.float32),right_alpha)
